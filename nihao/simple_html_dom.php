...
function file_get_html($url, $use_include_path = false, $context=null, $offset = -1, $maxLen=-1, $lowercase = true, $forceTagsClosed=true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
{
    ...
    // $contents = login_get_html($url);//file_get_contents($url, $use_include_path, $context, $offset);
    $contents = login_get_html($url);
...
function curl_get_contentsx($url)
{
        // $proxy = array_filter(array_map('trim',explode("\n", file_get_contents('proxy.txt'))));

        // $referer = array("http://http://university.which.co.uk/","http://university.which.co.uk/search/institution");
        $referer = parse_url($url);
        // $_referer = $referer[array_rand($referer)];
        // $_proxy = $proxy[array_rand($proxy)];
        ...
        $header[] = "Pragma: ";
        // $tmp = explode(":",$proxy[array_rand($proxy)]);
        // $_proxy = $tmp[0];
        // $_proxyport = $tmp[1];
        $ch = curl_init();
        ...
        curl_setopt($ch, CURLOPT_REFERER, $referer['host']);
        // curl_setopt($ch, CURLOPT_PROXYPORT,$_proxyport); 
...
class simple_html_dom
{
    ...
    function save_path($save_dir='',$filename='')
    {
